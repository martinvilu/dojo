...
<!-- estado:inicio -->
## Estado de Implementación (actualizado al 24/09/2026)

Cada propuesta incluye una línea **Estado** verificada contra el código de `main`:
✅ Implementada · 🟡 Parcial (existe una base que cubre parte de la propuesta) · ⬜ Pendiente.

| Categoría | ✅ | 🟡 | ⬜ |
|---|:---:|:---:|:---:|
| 1. Frontend, UX/UI y Accesibilidad | 3 | 7 | 40 |
| 2. Arquitectura y Calidad de Código | 9 | 6 | 35 |
| 3. Backend, Firebase y Seguridad | 6 | 8 | 36 |
| 4. Rendimiento y Web Vitals | 6 | 1 | 43 |
| 5. Testing, CI/CD y Observabilidad | 6 | 6 | 38 |
| **Total** | **30** | **28** | **192** |

**Cambios en el diagnóstico desde la versión original del documento:**
- Las Cloud Functions ya no viven en un único archivo: están separadas por dominio en `functions/src/modules/` y la callable `api` aplica autorización por acción (`functions/src/authz.js`) antes de despachar.
- Existe validación con Zod en el cliente (`src/lib/schemas/`, `src/lib/env.ts`); falta extenderla a las Cloud Functions.
- La auditoría de seguridad del 24/09/2026 (ver `status.md`) corrigió escalada de privilegios, fugas de secretos de curso a estudiantes, SSRF en Moodle y una RCE en Next.js; `npm audit` queda en 0.
- Siguen vigentes los monolitos de UI (`CourseSchedulesPanel.tsx` ~1.100 líneas, `AssignmentsPanel.tsx` ~630), el uso extendido de `any` (~440) y la ausencia de Playwright, Sentry y tests de componentes.
<!-- estado:fin -->
...
- **Estado (24/09/2026):** 🟡 Parcial — BaseModal y CommandPalette cierran con Escape; faltan modales propios.
...
- **Estado (24/09/2026):** 🟡 Parcial — Avatares migrados a `next/image`; quedan 6 `<img>` en componentes.
...
- **Estado (24/09/2026):** 🟡 Parcial — `aria-expanded` presente en 3 controles colapsables.
...
- **Estado (24/09/2026):** 🟡 Parcial — 17 elementos con `focus-visible`; falta estandarizar el resto.
...
- **Estado (24/09/2026):** 🟡 Parcial — Regiones `aria-live` en AuthScreen y algunos paneles.
...
- **Estado (24/09/2026):** 🟡 Parcial — Sidebar colapsable con breakpoints `md:`; sin drawer móvil dedicado.
...
- **Estado (24/09/2026):** ✅ Implementada — CommandPalette abre con ⌘K / Ctrl+K (`src/components/dashboard/ui/CommandPalette.tsx`).
...
- **Estado (24/09/2026):** ✅ Implementada — AuthScreen permite mostrar/ocultar la contraseña.
...
- **Estado (24/09/2026):** 🟡 Parcial — `cloneCourseExtraData` copia tareas entre cátedras.
...
- **Estado (24/09/2026):** ✅ Implementada — Descarga `.ics` en CalendarPanel y feed de suscripción `/api/calendar`.
...
- **Estado (24/09/2026):** ✅ Implementada — `src/modules/course/hooks/useCourseAttendance.ts`.
...
- **Estado (24/09/2026):** ✅ Implementada — `src/modules/course/hooks/useStudentRisk.ts`.
...
- **Estado (24/09/2026):** 🟡 Parcial — Schemas Zod en `src/lib/schemas/` (auth, course, moodle) y `src/lib/env.ts`; las Cloud Functions aún no validan con Zod.
...
- **Estado (24/09/2026):** ✅ Implementada — `src/components/ModuleErrorBoundary.tsx` envuelve los subtabs del dashboard.
...
- **Estado (24/09/2026):** 🟡 Parcial — Wrapper único `src/lib/api.ts` sobre la callable `api`, sin tipado por acción.
...
- **Estado (24/09/2026):** ✅ Implementada — `src/lib/dates.ts` con tests en `tests/unit/dates.test.ts`.
...
- **Estado (24/09/2026):** 🟡 Parcial — Roles y estados tipados en `src/modules/*/types.ts`; sin módulo de constantes único.
...
- **Estado (24/09/2026):** ✅ Implementada — `tsconfig.json` con `"strict": true` (persisten ~440 usos de `any`).
...
- **Estado (24/09/2026):** 🟡 Parcial — Queda 1 `console.log`; `src/lib/logger.ts` disponible.
...
- **Estado (24/09/2026):** ✅ Implementada — `src/modules/github/types.ts`.
...
- **Estado (24/09/2026):** ✅ Implementada — `requireBearerUser` / `requireBearerProfile` / `requireCourseSubscriptionToken` en `src/app/api/middleware/api.ts`.
...
- **Estado (24/09/2026):** ✅ Implementada — `src/hooks/useDebounce.ts`.
...
- **Estado (24/09/2026):** 🟡 Parcial — Helper `jsonError` compartido por las rutas App Router.
...
- **Estado (24/09/2026):** ✅ Implementada — `src/lib/clipboard.ts` con fallback.
...
- **Estado (24/09/2026):** 🟡 Parcial — Acciones extraídas a `useAdminPanel`; el componente sigue en ~480 líneas.
...
- **Estado (24/09/2026):** ✅ Implementada — Cloud Functions organizadas en `functions/src/modules/<dominio>/`.
...
- **Estado (24/09/2026):** 🟡 Parcial — Reglas validan tipos/longitudes en foros y encuestas; el resto se opera solo vía backend.
...
- **Estado (24/09/2026):** ✅ Implementada — Autorización por acción en `functions/src/authz.js`; reglas impiden cambiar `role` desde el cliente.
...
- **Estado (24/09/2026):** ✅ Implementada — Todas las funciones usan `firebase-functions/v2`.
...
- **Estado (24/09/2026):** 🟡 Parcial — La capa `authz` valida identificadores y membresía antes de cada handler.
...
- **Estado (24/09/2026):** 🟡 Parcial — `gradeSubmission` registra cada calificación en `audit_logs`.
...
- **Estado (24/09/2026):** ✅ Implementada — Whitelist de orígenes en `src/lib/cors.ts` con tests.
...
- **Estado (24/09/2026):** 🟡 Parcial — Las reglas prohíben editar/borrar mensajes del foro (solo reacciones y mejor respuesta).
...
- **Estado (24/09/2026):** ✅ Implementada — Verificación HMAC SHA-256 (`src/lib/github/webhookVerification.ts`).
...
- **Estado (24/09/2026):** 🟡 Parcial — Campo `archived` en cursos y tareas; `deleteCourse` sigue borrando físicamente.
...
- **Estado (24/09/2026):** 🟡 Parcial — Suite de reglas con el emulador de Firestore en CI (`npm run test:rules`).
...
- **Estado (24/09/2026):** 🟡 Parcial — El dispatcher propaga códigos `HttpsError` (`permission-denied`, `not-found`, `invalid-argument`).
...
- **Estado (24/09/2026):** 🟡 Parcial — Los ayudantes no pueden modificar la planificación base (`updateCourseSettings`).
...
- **Estado (24/09/2026):** ✅ Implementada — Motor de similitud por k-gramas/Jaccard en `functions/src/modules/github/plagiarism/`.
...
- **Estado (24/09/2026):** ✅ Implementada — Paneles secundarios cargados con `next/dynamic`.
...
- **Estado (24/09/2026):** ✅ Implementada — Fuentes con `next/font` en `src/app/layout.tsx`.
...
- **Estado (24/09/2026):** ✅ Implementada — Búsquedas con `useDebounce`.
...
- **Estado (24/09/2026):** ✅ Implementada — Avatar con `next/image` y dimensiones fijas.
...
- **Estado (24/09/2026):** ✅ Implementada — Media query `prefers-reduced-motion` en `globals.css`.
...
- **Estado (24/09/2026):** 🟡 Parcial — Formateadores de fecha cacheados en los paneles de GitHub.
...
- **Estado (24/09/2026):** ✅ Implementada — CommandPalette indexado con `Map`.
...
- **Estado (24/09/2026):** 🟡 Parcial — Tests nativos para `dates`, `env`, `cors` y verificación de webhooks.
...
- **Estado (24/09/2026):** ✅ Implementada — `tests/firestore.rules.test.js` contra el emulador.
...
- **Estado (24/09/2026):** ✅ Implementada — `actions/setup-node` con caché npm en todos los jobs.
...
- **Estado (24/09/2026):** ✅ Implementada — Jobs paralelos: frontend, functions, reglas y Selenium.
...
- **Estado (24/09/2026):** ✅ Implementada — Validación Zod de variables de entorno en `src/lib/env.ts`.
...
- **Estado (24/09/2026):** 🟡 Parcial — Mocks de Firebase Admin en la suite Jest de `functions/`.
...
- **Estado (24/09/2026):** 🟡 Parcial — Páginas `public/test-lti.html` y `public/fake-moodle-return.html` para probar LTI.
...
- **Estado (24/09/2026):** 🟡 Parcial — `npm audit` limpio (0 vulnerabilidades) pero sin chequeo automático en CI.
...
- **Estado (24/09/2026):** 🟡 Parcial — `seed.sh` y `functions/seed.js`.
...
- **Estado (24/09/2026):** ✅ Implementada — 19 suites / 176 tests Jest en `functions/tests/`.
...
- **Estado (24/09/2026):** 🟡 Parcial — Solo se define `frame-ancestors *` (necesario para embeber en Moodle).
...
- **Estado (24/09/2026):** ✅ Implementada — Diagramas Mermaid en `docs/UML.md` y `docs/ARCHITECTURE.md`.
