...
export function StudentNinjaRankCard({
  ...
  // Pre-compute points and stats to avoid O(N) multi-pass filtering on every render
  const stats = useMemo(() => {
    const pid = profile?.id;
    if (!pid) {
      return {
        totalXp: 0, currentLevel: 1, currentLevelProgress: 0,
    ...
    const totalXp = (activeNinjaCount * 10) + (bestAnswerCount * 100) + (presentCount * 10) + submissionPoints;

    return {
      totalXp,
      currentLevel: Math.floor(totalXp / 100) + 1,
      currentLevelProgress: totalXp % 100,
      hasChakraMaster: (gradesCount > 0 ? gradesSum / gradesCount : 0) >= 9,
      hasPerfectAttendance: totalClasses >= 3 && presentCount === totalClasses,
      hasActiveNinja: activeNinjaCount >= 3,
      hasSolucionador: bestAnswerCount > 0
    };
  }, [profile?.id, courseComments, courseAttendance, submissions]);

  const {
    totalXp, currentLevel, currentLevelProgress,
    hasChakraMaster, hasPerfectAttendance,
    hasActiveNinja, hasSolucionador
  } = stats;
