                                          ...
                                          {githubActivityLoading ? (
                                            ...
                                            <div className="space-y-4">
                                              ...
                                              <div className="flex border-b border-neutral-900 pb-2 gap-4">
                                                ...
                                                <button
                                                  type="button"
                                                  onClick={() => setGithubActivityTab("visualizer")}
                                                  className={`font-semibold pb-1 border-b-2 transition-colors cursor-pointer ${
                                                    githubActivityTab === "visualizer"
                                                      ? "border-emerald-500 text-emerald-400"
                                                      : "border-transparent text-gray-400 hover:text-white"
                                                  }`}
                                                >
                                              ...
                                              {githubActivityTab === "visualizer" && (() => {
                                                const totalCommits = githubActivityData.commits.length;
                                                const stats: Record<string, { name: string, count: number, avatar: string }> = {};
                                                githubActivityData.commits.forEach((c: any) => {
                                                ...
                                                return (
                                                  <div className="space-y-4">
                                                    <div className="bg-neutral-900/60 p-4 rounded-xl border border-neutral-850 space-y-3 text-left">
                                                      <span className="text-xs font-bold text-gray-300 uppercase tracking-wider block">Distribución del Trabajo (Commits)</span>
                                                          ...
                                                          return (
                                                            <div key={login} className="space-y-1">
                                                              <div className="flex justify-between items-center text-[10px]">
                                                                ...
                                                                <span className="text-gray-400 font-bold">{authorData.count} commits ({percentage}%)</span>
                                                              </div>
                                                    ...
                                                    <div className="bg-neutral-900/60 p-4 rounded-xl border border-neutral-850 space-y-3 text-left">
                                                      <span className="text-xs font-bold text-gray-300 uppercase tracking-wider block">Flujo de Ramas y Commits</span>
                                                      <div className="space-y-3 max-h-52 overflow-y-auto pr-1">
                                                        {githubActivityData.commits.map((c: any, index: number) => {
                                                          ...
                                                          return (
                                                            <div key={index} className="flex items-start space-x-3 text-[10px]">
                                                              <div className="flex flex-col items-center pt-1 w-12 relative">
                                                                ...
                                                                {index < githubActivityData.commits.length - 1 && (
                                                              ...
                                                              <div className="flex-1 bg-neutral-950/45 p-2 rounded-lg border border-neutral-900 space-y-1">
                                                                ...
                                                                <div className="flex items-center space-x-1 pt-1 text-[9px] text-gray-500 font-sans">
                                                                  <span>Autor:</span>
                                                                  <img src={c.author_avatar} alt={c.author} className="w-3.5 h-3.5 rounded-full object-cover" />
                                                                  <span className="text-gray-400">{c.author} (@{c.author_login})</span>
                                                                </div>
                                                              </div>
                                                            </div>
                                                          );
                                  ...
                                  {studentGithubActivity ? (
                                    <div className="space-y-4 text-left">
                                      ...
                                      <div className="flex border-b border-neutral-900 pb-2 gap-4">
                                        ...
                                        <button
                                          type="button"
                                          onClick={() => setStudentGithubActivityTab("visualizer")}
                                          className={`font-semibold pb-1 border-b-2 transition-colors cursor-pointer ${
                                            studentGithubActivityTab === "visualizer"
                                              ? "border-emerald-500 text-emerald-400"
                                              : "border-transparent text-gray-400 hover:text-white"
                                          }`}
                                        >
                                      ...
                                      {studentGithubActivityTab === "visualizer" && (() => {
                                        const totalCommits = studentGithubActivity.commits.length;
                                        const stats: Record<string, { name: string, count: number, avatar: string }> = {};
                                        studentGithubActivity.commits.forEach((c: any) => {
                                          const key = c.author_login || 'desconocido';
                                          if (!stats[key]) {
                                            stats[key] = {
                                              name: c.author || 'Desconocido',
                                        ...
                                        return (
                                          <div className="space-y-4">
                                            ...
                                            <div className="bg-neutral-900/60 p-4 rounded-xl border border-neutral-850 space-y-3 text-left">
                                              <span className="text-xs font-bold text-gray-300 uppercase tracking-wider block">Flujo de Ramas y Commits</span>
                                              <div className="space-y-3 max-h-52 overflow-y-auto pr-1">
                                                {studentGithubActivity.commits.map((c: any, index: number) => {
                                                  const branchColorMap: Record<string, { dot: string, bg: string, text: string }> = {
                                                    'main': { dot: 'bg-emerald-500', bg: 'bg-emerald-950/20', text: 'text-emerald-400 border-emerald-800/40' },
                                                    'dev': { dot: 'bg-blue-500', bg: 'bg-blue-950/20', text: 'text-blue-400 border-blue-800/40' },
                                                    'feature/alerts': { dot: 'bg-amber-500', bg: 'bg-amber-950/20', text: 'text-amber-400 border-amber-800/40' }
                                                  ...
                                                  return (
                                                    <div key={index} className="flex items-start space-x-3 text-[10px]">
                                                      <div className="flex flex-col items-center pt-1 w-12 relative">
                                                        ...
                                                        {index < studentGithubActivity.commits.length - 1 && (
                                                          <div className="w-0.5 h-7 bg-neutral-800 mt-1 absolute top-2.5"></div>
                                                        )}
                                                      </div>
