...
import { CourseOverviewPanelProps, StudentRiskData } from "../types";
import { useStudentRisk, StudentWithCommission } from "../hooks/useStudentRisk";
import { formatDateSafe, formatShortDate } from "@/lib/dates";
...
export function CourseOverviewPanel({
  ...
  loadingOverviewSubmissions = false,
  roster = [],
  courseAttendance = [],
  courseSubmissions = [],
  ...
  const typedRoster = roster as StudentWithCommission[];
  const { studentsRiskData, studentsAtRisk } = useStudentRisk(
    typedRoster,
    courseAttendance,
    courseSubmissions,
    assignments,
  ...
  // Memoize pending submissions and unanswered forum comments for O(1) rendering
  const pendingSubmissions = useMemo(() => {
    return (overviewSubmissionsList as unknown as EnrichedSubmission[]).filter(
      (s) => s.status === "submitted" && (s.grade === undefined || s.grade === "" || s.grade === null)
    );
  }, [overviewSubmissionsList]);

  const activeForumConsultations = useMemo(() => {
    return courseComments.filter((c) => c.user_role !== "teacher" && !c.is_best_answer);
  }, [courseComments]);
    ...
    <div className="space-y-6 animate-fade-in font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-neutral-900/60 border border-neutral-800 p-5 rounded-2xl flex items-center justify-between shadow-lg">
          <div className="space-y-1">
            <span className="text-[10px] text-gray-550 font-bold uppercase tracking-widest font-sans">
        ...
        <div className="bg-neutral-900/60 border border-neutral-800 p-5 rounded-2xl flex items-center justify-between shadow-lg">
          <div className="space-y-1">
            ...
            <div className="text-3xl font-black text-emerald-500 font-mono">
              {overviewSubmissionsList.length}
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {loadingOverviewSubmissions ? (
            ...
            <div className="space-y-3 overflow-y-auto flex-1 pr-1 custom-scrollbar">
              {pendingSubmissions.map((sub) => {
                ...
                const studentComm = (sub.profiles?.commissions && sub.profiles.commissions[courseId]) || "";
                return (
                  <div
                    key={sub.id}
        ...
        <div className="bg-neutral-900/60 p-6 rounded-2xl border border-neutral-800 space-y-4 shadow-xl flex flex-col max-h-[500px]">
          ...
          <div className="space-y-3 overflow-y-auto flex-1 pr-1 custom-scrollbar">
            {activeForumConsultations.slice(0, 5).map((comment) => (
              <div
                key={comment.id}
              ...
              >
                <div className="flex justify-between items-center text-[10px]">
                  ...
                  <span className="text-gray-550 font-sans">
                    {formatDateSafe(comment.created_at, formatShortDate)}
                  </span>
                </div>
                ...
                <div className="flex justify-end pt-1">
                  <button
                    type="button"
                    onClick={() => {
                      setExpandedComments((prev: Record<number, boolean>) => ({ ...prev, [comment.classNumber]: true }));
            ...
            {activeForumConsultations.length === 0 && (
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-neutral-900 text-gray-300">
              {studentsAtRisk.slice(0, 5).map((student) => {
                const riskData: StudentRiskData = studentsRiskData.get(student.id) || {
                ...
                return (
                  <tr key={student.id} className="hover:bg-neutral-950/20 transition-colors">
                    ...
                    <td className="py-3 text-gray-400 font-mono">
                      {(student.commissions && student.commissions[courseId]) || "Sin Comisión"}
                    </td>
