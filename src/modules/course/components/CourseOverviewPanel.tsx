...
export function CourseOverviewPanel({
  ...
  // ⚡ Bolt Optimization: Memoize pending submissions filtering to avoid O(N) operations on every render
  ...
  // ⚡ Bolt Optimization: Memoize active consultations to prevent redundant O(N) filtering across JSX
  const activeConsultations = React.useMemo(() => {
    return courseComments.filter((c: any) => c.user_role !== "teacher" && !c.is_best_answer);
  }, [courseComments]);

  // ⚡ Bolt Optimization: Precompute at-risk students array to reduce O(N) lookups during render loops
