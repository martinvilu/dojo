...
## 2023-11-09 - O(4N) Kanban render filtering operations
**Learning:** Found a performance bottleneck in `src/modules/course/components/CourseSchedulesPanel.tsx` where four separate `O(N)` `.filter()` and `.map()` sweeps were executed synchronously inside an Immediately Invoked Function Expression (IIFE) within the render cycle of the Kanban board, causing `O(4N)` redundant object instantiations on every drag-and-drop render update.
**Action:** When mapping UI elements into categorized columns or sections, avoid using inline IIFEs and repeated `.filter()` operations over the entire dataset per column. Pre-compute a single mapped categorized state in a `useMemo` block that makes one `O(N)` pass, effectively reducing render lookups per column to `O(1)`. Always check for staged `pnpm-lock.yaml` files before committing optimizations.
