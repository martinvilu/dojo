...
## 2025-02-18 - Preserving exact semantic checks in O(1) loop refactors
**Learning:** When refactoring nested `filter` operations over Object keys (e.g., `courseAttendance.filter((c) => Boolean(c.records[student.id]))`) into O(1) hash map pre-computations using `Object.entries()`, it's critical to preserve the exact truthiness semantics of the original logic. A blind `forEach` over entries will count keys with explicitly falsy values, causing a semantic regression.
**Action:** Always explicitly check the original boolean condition (`if (!status) return;`) when translating inline loop filters into pre-computed hash maps.
