...
## 2023-10-27 - O(N) array filtering within nested loops inside useStudentRisk
**Learning:** Found an O(R * (A + S)) performance bottleneck in `useStudentRisk.ts` where arrays of course attendances ($A$) and course submissions ($S$) were being `.filter()`'d inside a `.forEach()` loop iterating over the student roster ($R$). This led to significant performance degradation during rapid re-renders of the Course Students / Overview panel when data sizes scaled.
**Action:** Replace inline array `.filter()` inside loops by pre-computing attendance statistics and submission lookups into Hash Maps/Sets before the `roster.forEach` iteration, turning the complexity into $O(R + A + S)$ with $O(1)$ internal lookups.
