...
## 2023-10-27 - Inline array filtering during JSX render block mapping and length checks
**Learning:** In components rendering dashboard items or long lists (like `CourseOverviewPanel`), performing `array.filter(...)` operations directly inline inside JSX to both `.map()` the filtered results AND to conditionally render fallbacks using `.filter(...).length === 0` duplicates the exact same O(N) filtering cost across multiple nodes per render tick.
**Action:** Always extract repeated inline `.filter()` operations over array props into precomputed variables wrapped in `React.useMemo` to ensure the list is only filtered once per dependency change and can be safely referenced for both mapping and length checking.
