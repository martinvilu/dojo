...
## 2025-10-24 - Batching Multiple Filters into Single-Pass Memoized Block
**Learning:** React component functional performance degraded by having multiple duplicate `filter()` and `forEach()` statements calculating derived properties (e.g., in `StudentNinjaRankCard.tsx`).
**Action:** Replace sequential filter/forEach arrays spanning the same target with single-pass `for` loops inside a solitary `useMemo` block to memoize intermediate UI state computations.
