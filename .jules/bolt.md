...
## 2026-09-05 - Avoid redundant array filtering in React renders
**Learning:** In large React components rendering lists (like `CourseOverviewPanel.tsx`), inline `.filter()` methods on arrays directly within the return block run every render. When multiple conditions require slightly different sub-lists from the same source, they cause redundant O(N) calculations and hurt rendering performance.
**Action:** Extract list-filtering logic into a single `useMemo` block at the top of the component to pre-calculate required filtered states (`pendingSubmissions`, `atRiskStudents`, etc.) so they are only recalculated when the underlying data changes, not on every layout shift or state update.
