...
## 2026-09-08 - O(N*M) nested iterations in useMemo hook in CourseOverviewPanel
**Learning:** The `useMemo` hook in `CourseOverviewPanel` performed deeply nested `.filter()` and `.some()` calls inside a loop over the `roster` to calculate missing assignments and attendance rates. Since `useMemo` recalculates during specific dependency updates, executing \(O(R \times (A + S))\) operations per render causes severe performance regressions for large classes with many assignments.
**Action:** Replace nested array scanning inside render components with pre-computed \(O(1)\) Hash Maps or Sets. Iterating over relationships (like attendance records or submissions) once to construct maps provides significant optimization.
