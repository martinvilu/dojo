...
## 2025-02-28 - Optimize array derivations in StudentNinjaRankCard
**Learning:** The `StudentNinjaRankCard.tsx` component exhibited unnecessary O(N) recalculations on every render by executing multiple array filtering (`.filter()`) and reduction methods over `courseComments`, `courseAttendance`, and `submissions`. This resulted in excessive CPU overhead for larger arrays because derivations were re-evaluated synchronously regardless of whether the dependencies actually mutated.
**Action:** Replaced functional array sweeps (`.filter()` chains) with traditional, single-pass `for` loops and memoized the entire calculation block using `useMemo` hooked to array dependencies, significantly reducing rendering bottleneck.
